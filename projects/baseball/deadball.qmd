...
ui <- fluidPage(
  sidebarPanel(
    ...
    column(3,tableOutput("scorecard"),textOutput("score"), textOutput("roll"), textOutput("innings")),
    column(4, textOutput("outs"), textOutput("outcome")),
    column(5,div(style = "
                   margin-left:36%;
  ...
  server <- function(input, output) {
    ...
    output$enable1 <- reactive({if(1 %in% unlist(rv$x[9]) | 1 %in% unlist(rv$y[9])){0}})
    outputOptions(output, 'enable1', suspendWhenHidden=FALSE)
    output$enable2 <- reactive({if(2 %in% unlist(rv$x[9]) | 2 %in% unlist(rv$y[9])){0}})
